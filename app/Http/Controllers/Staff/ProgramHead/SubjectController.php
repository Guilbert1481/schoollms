...
class SubjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // 3. Prepare and Save — explicit whitelist (A3): $request->all() would let
        //    the client mass-assign school_id / created_by (cross-tenant write),
        //    so those are set server-side and only form fields pass through.
        $data = $request->only(['code', 'description', 'category', 'academic_level_id']);
        $data['name'] = $name;
        $data['scope'] = $scope;
        $data['is_basic_ed'] = 0;
