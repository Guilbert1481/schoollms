...
class EnrollmentController extends Controller
{
    ...
    protected function resolveSemesterNumber(?\App\Models\Term $term): ?int
    {
        ...
        if (str_contains($raw, 'summer')) {
            return 3;
        }
        if (str_contains($raw, 'first') || str_starts_with($raw, '1')) {
            return 1;
        }
        if (str_contains($raw, 'second') || str_starts_with($raw, '2')) {
            return 2;
        }
        if (str_contains($raw, 'third') || str_starts_with($raw, '3')) {
            return 3;
        }
        if (str_contains($raw, 'fourth') || str_starts_with($raw, '4')) {
            return 4;
        }
     ...
     */
    public function store(Request $request, $termId, \App\Services\Uploads\SecureUpload $secureUpload)
    {
        if (! auth()->check()) {
            return $request->ajax()
        ...
        if ($request->hasFile('student_photo') && blank($student->photo_path)) {
            // Profile photo: re-encoded to strip any embedded payload (H3),
            // public disk (it's a UI avatar).
            $student->photo_path = $secureUpload->storeImage(
    ...
    public function storePathway(Request $request, $termId, \App\Services\Uploads\SecureUpload $secureUpload)
    {
        $term = Term::findOrFail($termId);
