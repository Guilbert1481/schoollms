...
class TFController extends Controller
{
    ...
    {logger('QB session type: ' . session('qb.question_type'));
        // Only allow access if metadata is set and question_type === 'true_false'
        if (!$this->hasMetadata() || $this->getQuestionType() !== 'true_false') {
            return redirect()
                ->route('teacher.question.metadata')
                ->withErrors('Please complete question metadata first (and select True / False as the type).');
        }

        $meta = session('qb');

        return view(
            'teacher.question-bank.question-types.tf',
            compact('meta')
        );
    }

    /**
     * SAVE TF QUESTIONS (BATCH)
     */
    public function saveTf(Request $request)
    {
        // Only allow saving if session metadata is valid and question_type is 'true_false'
        if (!$this->hasMetadata() || $this->getQuestionType() !== 'true_false') {
            return response()->json([
                'success' => false,
                'error'   => 'Metadata session missing or not set to True / False.',
            ], 422);
        }
        ...
        $request->validate([
            ...
            'questions.*.correct_index'         => 'required|integer|in:0,1',
        ]);
        ...
        try {
            foreach ($request->questions as $q) {
                ...
                // SAVE ANSWER (choices table, two rows: True and False)
                $question->choices()->create([
                    'choice_text' => 'True',
                    'is_correct'  => $q['correct_index'] === 0,
                ]);
                $question->choices()->create([
                    'choice_text' => 'False',
                    'is_correct'  => $q['correct_index'] === 1,
                ]);
            }
            ...
            return response()->json([
                'success' => false,
                'error'   => 'Failed to save True/False questions.',
            ], 500);
        }
    }
    ...
    {
        // Check required session keys for qb
        return session()->has([
            'qb.subject_id',
            'qb.topic_id',
            'qb.lesson_id',
