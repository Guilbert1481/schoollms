// public/js/tests/tf.js
// TF BUILDER – QUESTION BANK
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!questionsContainer || !addQuestionBtn || !saveBtn) {
        console.warn('TF builder elements missing');
        return;
    }
    ...
    window.addEventListener('beforeunload', () => {
        if (!isSaving) clearQuestionSession();
    });
        ...
        card.innerHTML = `
            ...
            <div class="question-radio-input">
                ...
                <div class="options-container">
                    ${renderOption(uid, "True")}
                    ${renderOption(uid, "False")}
                </div>
    ...
    function renderOption(uid, label) {
        return `
            <div class="option-row">
                <div class="option-marker">
                    <input type="radio" name="correct_${uid}">
                </div>
                <input type="text" class="option-input" value="${label}" readonly>
            </div>
        `;
    }
    ...
    saveBtn.addEventListener('click', async (e) => {
        ...
        cards.forEach(card => {
            ...
            const radios = card.querySelectorAll('.option-row input[type="radio"]');
            radios.forEach((radio, idx) => {
                if(radio.checked) correct_index = idx;
            });

            if (correct_index !== null && question_text) {
                questions.push({
                    question_text,
                    keyword,
                    difficulty,
                    explanation,
                    correct_index // 0 = True, 1 = False
                });
            }
        });
        ...
        try {
            const res = await fetch('/teacher/tests/tf/save', {
                method: 'POST',
                headers: {
