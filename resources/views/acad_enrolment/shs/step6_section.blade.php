...
<form method="POST" action="{{ route('public.apply.shs.step6.store', $term->id) }}">
    @csrf
    @if ($sections->isEmpty())
        <div style="padding:24px;background:#fef3c7;border:1px solid #fcd34d;border-radius:10px;color:#92400e;">
            No sections have been published for this strand yet. You may continue without selecting one — the
    ...
    <div class="form-footer" style="display:flex;justify-content:space-between;margin-top:32px;padding-top:20px;border-top:1px solid #e2e8f0;">
        <a href="{{ route('public.apply.shs.step5', $term->id) }}" class="btn btn-back">Back</a>
