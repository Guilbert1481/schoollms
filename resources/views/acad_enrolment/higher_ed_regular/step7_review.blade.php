...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:18px;display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:14px;">
    ...
    <div><strong>Year &amp; Sem:</strong> Year {{ $program['year_level'] }} · Sem {{ $program['semester'] }}</div>
    <div><strong>Education Level:</strong> {{ ucfirst($program['education_level']) }}</div>
    <div><strong>Enrollee Type:</strong> {{ ucfirst($program['enrollee_type']) }}</div>
    <div><strong>Program Type:</strong> {{ ucfirst($program['program_type']) }}</div>
    <div><strong>Block Section:</strong> {{ $section?->name ?? '—' }} ({{ $section?->code ?? '' }})</div>
...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:0;font-size:14px;overflow:hidden;">
    ...
    <table style="width:100%;border-collapse:collapse;">
        ...
        <tbody>
            ...
            @empty
                <tr><td colspan="4" style="padding:24px;text-align:center;color:#94a3b8;">No classes attached to this section yet.</td></tr>
            @endforelse
        </tbody>
    </table>
    </div>
...
<form method="POST" action="{{ route('public.apply.higher_regular.submit', $term->id) }}" style="margin-top:32px;">
    ...
    <div class="form-footer" style="display:flex;justify-content:space-between;padding-top:20px;border-top:1px solid #e2e8f0;">
        <a href="{{ route('public.apply.higher_regular.step6', $term->id) }}" class="btn btn-back">Back</a>
        <button type="submit" class="btn btn-next" @disabled($classes->isEmpty())>Submit Enrolment</button>
    </div>
</form>
@endsection
