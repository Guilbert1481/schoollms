{{-- views/components/charts/trend-card.blade.php --}}

@props([
    'title' => 'Trend',
    'id'    => 'trend',
    'type'  => 'line',    // line|bar|pie... (whatever your JS supports)
    'color' => 'indigo',  // tailwind color name without shade
])
...
@php
    ...
    // Canvas id used by JS to mount the chart
    $canvasId = 'chart_' . $id;
@endphp
...
<div class="{{ $cardClasses }} {{ $borderClasses }} p-6 rounded-3xl transition-all duration-300 hover:shadow-md group">
    ...
    <div class="mt-6 pt-4 border-t border-slate-50 dark:border-slate-800 flex justify-between items-center">
        ...
        <button type="button"
            class="text-[10px] font-black text-{{ $accentColor }}-600 hover:text-{{ $accentColor }}-700 transition uppercase tracking-widest">
            Details →
        </button>
    </div>
</div>