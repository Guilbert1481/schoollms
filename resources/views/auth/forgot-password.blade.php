...
<div class="min-h-screen flex items-center justify-center p-0 md:p-6">
    <div class="flex w-full max-w-5xl min-h-[640px] overflow-hidden bg-white shadow-2xl md:rounded-[2.5rem]">
        ...
        <div class="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-white">
            <div class="mb-8">
                <h2 class="text-4xl font-serif-premium text-slate-900 mb-2">Forgot Password?</h2>
            ...
            @if (session('status'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center gap-3 text-emerald-600">
                    <i data-lucide="mail-check" class="w-5 h-5 flex-shrink-0"></i>
                    <p class="text-[10px] font-black uppercase tracking-wider">{{ session('status') }}</p>
                </div>
            @endif
